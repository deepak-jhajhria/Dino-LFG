  ...
  return (
    <nav className="px-2 mt-5 sm:mt-7">
      <div className="container px-0 max-w-[1140px] flex items-center justify-between bg-white rounded-full relative z-10 border-black border-b-4">
        <Link to={"/"} className='font-Lucky text-3xl sm:text-4xl md:text-[39.13px] group font-normal leading-[37.13px] text-[#FBA11D] ml-3 sm:ml-6 drop-shadow-[0px_1.3px_0px_#000000]'>
        ...
        <div className="flex items-center justify-between lg:w-[65%] sm:mr-2 l-3">
          <ul className={`${isOpen ? "right-0" : "-right-full"} flex items-center gap-7 mobileview`}>
            <li><Link to={"/about"} onClick={() => setOpen(false)} className="relative text-lg font-normal text-black duration-500 navlinks group font-Chewy">About<NavLinks /></Link></li>
            <li><Link to={"/"} onClick={() => setOpen(false)} className="relative text-lg font-normal text-black duration-300 navlinks group font-Chewy">Tokenomic<NavLinks /></Link></li>
            <li><Link to={"/"} onClick={() => setOpen(false)} className="relative text-lg font-normal text-black duration-300 navlinks group font-Chewy">Roadmap <NavLinks /></Link></li>
            <li><Link to={"/"} onClick={() => setOpen(false)} className="relative text-lg font-normal text-black duration-300 navlinks group font-Chewy">FAQ <NavLinks /></Link></li>
